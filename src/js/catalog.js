document.addEventListener('DOMContentLoaded', function () {
  ...
  function populateBestSets (products) {
    if (bestSetsListEl) {
      const setsOnly = products.filter((p) =>
        p.category?.toLowerCase().includes('sets')
      )
      const source = setsOnly.length > 0 ? setsOnly : products

      const randomItems = [...source]
  ...
  function populateCustomFilters (products) {
    ...
    const colors = [...new Set(products.map((p) => p.color))].sort((a, b) =>
      a.localeCompare(b)
    )
  ...
  function setupEventListeners () {
    ...
    hideFiltersButton.addEventListener('click', () => {
      ...
      hideFiltersButton.textContent = isHidden
        ? 'Hide filters'
        : 'Show filters'
    })
    ...
    filterGroups.forEach((group) => {
      ...
      if (defaultLi) {
        ...
        const defaultButtonText =
          defaultLi.querySelector('button')?.textContent || 'Choose option'
  ...
  function applyFiltersAndDisplay () {
    ...
    if (activeFilters.size !== 'default') {
      tempProducts = tempProducts.filter((p) =>
        p.size.includes(activeFilters.size)
      )
    }
    if (activeFilters.color !== 'default') {
      tempProducts = tempProducts.filter(
        (p) => p.color === activeFilters.color
      )
    }
  ...
  function updatePaginationControls (totalPages) {
    ...
    nextButton.style.display =
      currentPage === totalPages || totalPages === 0 ? 'none' : 'block'
  }
