document.addEventListener('DOMContentLoaded', function () {
  ...
  const checkoutBtn = document.querySelector(
    '.cart-actions__summary .checkout'
  )
  ...
  function renderSummary (cartDetails) {
    if (!summaryList || cartDetails.length === 0) {
      if (!summaryList) {
        console.error(
          "Eroare: Nu am găsit 'summaryList' (selectorul '.cart-actions__summary ul')"
        )
      }
      return
    }
    ...
    if (liItems.length === 4) {
      liItems[0].innerHTML = `<span>Sub total</span> <span>$${subtotal.toFixed(
        2
      )}</span>`
      liItems[1].innerHTML = `<span>Shipping</span> <span>$${shipping.toFixed(
        2
      )}</span>`
      liItems[2].innerHTML = `<span>Discount</span> <span>-$${discount.toFixed(
        2
      )}</span>`
      liItems[3].innerHTML = `<span>Total</span> <span>$${total.toFixed(
        2
      )}</span>`
